...
public abstract class SimpleGUIApp extends JPanel implements Runnable {
    ...
    private static JFrame makeNewFrame(SimpleGUIApp programToRun, String title) { return makeNewFrame(programToRun, title, false); }
    ...
    public static void start(SimpleGUIApp mainProgram, String name, boolean isUndecorated) { mainProgram.setFrame(makeNewFrame(mainProgram, name, isUndecorated)); }

    public static void start(SimpleGUIApp mainProgram, String name) { mainProgram.setFrame(makeNewFrame(mainProgram, name)); }
    ...
    public void drawWidget(Widget w) { _draw.drawOthercontext(w.draw(), w.pos()); }
    ...
    /** Calls DrawToScreen(), Timer.correctedDelay() with your target FPS in mind and cls(). This is the most convenient
