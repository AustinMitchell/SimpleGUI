...
/** Object that works as an overhead for the BufferedImage object. Has its own limited functionality, but if you wish to
 * work directly with the BufferedImage, you can get this from the Image object. Ths object's funcionality is currently
 * limited as it's meant to have only simple and easy to understand usage. **/
public final class Image {
    ...
    /** Returns a copy of an image object resized to new bounds. Scales pixels by deciding which pixel has the best
     * claim (which pixel is closest to the empty pixel upon resize). Does not perform any anti-aliasing. WARNING: If
     * you shrink an image and set that image to it, the old data is permanently lost.
     * @param image     Image object to resize.
     * @param newWidth  New width to scale image to.
     * @param newHeight New height to scale image to. **/
    public static Image resize(Image image, int newWidth, int newHeight) { return image.resize(newWidth, newHeight); }

    public static Image resizeScaledWidth(Image image, int newWidth) { return image.resizeScaledWidth(newWidth); }

    public static Image resizeScaledHeight(Image image, int newHeight) { return image.resizeScaledHeight(newHeight); }

    /** Returns a new image set to the new orientation given. **/
    public static Image reorient(Image image, Orientation orientation) { return image.reorient(orientation); }

    /** Returns an image rotated 90 degrees clockwise by rotationsRight many times, and sets new orientation. A negative
     * number means left turns. **/
    public static Image rotate(Image image, int rotationsRight) { return image.rotate(rotationsRight); }
    ...
    /** Returns the filename of the image. If it is an image created without a filename, then a default filename is
     * used: the String (image width) + "-by-" + (image height). **/
    ...
    /** Set the orientation of the image. This does not affect the image, but it will affect the output of
     * reorient() **/
    ...
    public Image(int width, int height) {
        ...
        _image       = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB); // set to TYPE_INT_ARGB to support
                                                                                      // transparency
    ...
    public Image(ConstIntVector2D dimensions) { this(dimensions.x(), dimensions.y()); }
     ...
     * @param copyImage If enabled, it will make a new BufferedImage by copying pixel data from the original. Otherwise,
     *                      it will use the same base object. **/
    ...
    /** Creates a new image from a given filename. First searches through working directory, then looks at the directoy
     * of the .class file. **/
    ...
    /** Returns whether an object equals this Image object. They must both be Image objects, and their color data must
     * match exactly. **/
    @Override
    public int hashCode() { return Arrays.hashCode(getPixelsNoCopy()); }
    ...
    public boolean equals(Object obj) {
        ...
        if (_size.x() != that.size().x())
            return false;
        if (_size.y() != that.size().y())
            return false;
    ...
    /** Returns a copy of the image resized to new bounds. Scales pixels by deciding which pixel has the best claim
     * (which pixel is closest to the empty pixel upon resize). Does not perform any anti-aliasing. WARNING: If you
     * skrink the image and set this image to it, the old data is permanently lost.
    ...
    /** Returns an image rotated 90 degrees clockwise by rotationsRight many times, and sets new orientation. A negative
     * number means left turns. */
    public Image rotate(int rotationsRight) { return reorient(Orientation.fromInt(Orientation.toInt(_orientation) + rotationsRight)); }
    ...
    private void verifyBounds(int x, int y) {
        if (x < 0 || x >= _size.x())
            throw new IndexOutOfBoundsException("x must be between 0 and " + (_size.x() - 1) + ", recieved " + x);
        if (y < 0 || y >= _size.y())
            throw new IndexOutOfBoundsException("y must be between 0 and " + (_size.y() - 1) + ", recieved " + y);
    }

    /** Returns the color data of the image at the pixel (w, h) as an integer. Note that the point (0, 0) is the origin
    ...
    /** Returns the color data of the image at the pixel (w, h) as an integer. Note that the point (0, 0) is the origin
     * of the image, not the screen.
     * @param pixel Coordinate of pixel to get */
    public int get(ConstIntVector2D pixel) { return get(pixel.x(), pixel.y()); }

    /** Returns the color data of the image at the pixel (w, h) as a Color object. Note that the point (0, 0) is the
     * origin of the image, not the screen.
    ...
    /** Returns the color data of the image at the pixel (w, h) as a Color object. Note that the point (0, 0) is the
     * origin of the image, not the screen.
     * @param pixel Coordinate of pixel */
    public Color getColor(ConstIntVector2D pixel) { return getColor(pixel.x(), pixel.y()); }

    /** Returns the color data of all the pixels as an array of integers. Note that this creates a copy of the original
     * pixels. */
    public int[] getPixels() {
        int[] pixelData = new int[_size.x() * _size.y()];
        System.arraycopy(((DataBufferInt) _image.getRaster().getDataBuffer()).getData(), 0, pixelData, 0, pixelData.length);
        return pixelData;
    }

    /** Returns the color data of all the pixels as an array of integers. Returns the base array. Changes to the
     * returned array WILL modify the image data. Important note: If Image is used in context of an ImageBox and not
     * just a standalone image, you need to take into account that the backing image used to preserve image quality
     * needs to be updated on changes made to the pixel array.
     * <P>
     * Note that this is essentially the fastest way to change pixel data directly. */
    public int[] getPixelsNoCopy() { return ((DataBufferInt) _image.getRaster().getDataBuffer()).getData(); }

    /** Sets the color data of the image at the pixel (w, h) using an integer. Note that th point (0, 0) is the origin
     * of the image, not the screen.
    ...
    /** Sets the color data of the image at the pixel (w, h) using an integer. Note that th point (0, 0) is the origin
     * of the image, not the screen.
     * @param pixel Coordinate of pixel to set
     * @param color Color to set pixel to */
    public void set(ConstIntVector2D pixel, int color) { set(pixel.x(), pixel.y(), color); }

    /** Sets the color data of the image at the pixel (w, h) using a Color object. Note that th point (0, 0) is the
     * origin of the image, not the screen.
     * @param x     x coordinate of pixel to set
     * @param y     y coordinate of pixel to set
     * @param color Color to set pixel to */
     ...
     * @param pixel Coordinate of pixel to set
     * @param color Color to set pixel to */
    public void setColor(ConstIntVector2D pixel, Color color) { setColor(pixel.x(), pixel.y(), color); }

    /** Sets all the color data of the image from an array of integers.
     * @param pixelData Pixel data buffer to assign to image */
    ...
    void draw(Graphics2D g, int x, int y) { g.drawImage(_image, x, y, null); }

    /** Draws an image to a Graphics2D object with the center of the image at point x, y on the screen.
     * @param g Graphics context to draw to
     * @param x Center x coordinate
     * @param y Center y coordinate */
    void drawCentered(Graphics2D g, int x, int y) { g.drawImage(_image, x - _image.getWidth() / 2, y - _image.getHeight() / 2, null); }

    /** Draws and image rotated by an angle to a Graphics2D object with the center of the image at point x, y on the
     * screen.
     * @param g     Graphics context to draw to
     * @param x     Center x coordinate
     * @param y     Center y coordinate
     * @param angle Angle to rotate image by in radians */
    void drawRotated(Graphics2D g, int x, int y, double angle) {
        g.rotate(angle, x, y);
        g.drawImage(_image, x - _image.getWidth() / 2, y - _image.getHeight() / 2, null);
        g.rotate(-angle, x, y);
    }
}